...
class TestIntegration:
    ...
    def test_add_product_with_zero_quantity_fails(self):
        ...
        # Создаем категорию, но не сохраняем в переменную, если не используем
        Category("Тест", "Тест")

        # Попытка создать товар с нулевым количеством
        with pytest.raises(ZeroQuantityError):
            Product("Плохой товар", "Нет в наличии", 100.0, 0)

        # Счетчик товаров не должен измениться, так как товар не создан
        # (это проверяется косвенно - если бы товар создался, счетчик увеличился бы)
